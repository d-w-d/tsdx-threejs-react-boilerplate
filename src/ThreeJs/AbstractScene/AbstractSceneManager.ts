...
import { ISceneEntity } from './AbstractSceneEntity';
...
export abstract class AbstractSceneManager {
  ...
  protected sceneEntities: ISceneEntity[] = [];
  ...
  async init() {
    ...
    if (!container) throw new Error('No container found with id: ' + this._containerId);
    ...
    await Promise.all(
      this.sceneEntities.map(async sceneEntity => {
  ...
  update() {
    ...
    this.sceneEntities.forEach(el => el.update(elapsedTime));
